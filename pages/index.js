...
"> Article Wali Website .</span></h1>
          ...
          {allPostsData.map(({ id, date, title, tag, color }) => (
           ...
           {/* <hr className="my-4" />
           <div className="grid grid-cols-4 gap-8 mt-0 sm:grid-cols-8 lg:grid-cols-12 sm:px-8 xl:px-0">
          {RapidNews.map(({ value, id }) => (
            <Link href={`/posts/${id}`}>
              <a key={id} className="relative flex flex-col col-span-4 px-0 py-0 space-y-2 overflow-hidden sm:rounded-xl">
              <div className={`py-1 px-2 w-min items-start text-sm font-medium text-red-700 bg-red-100 rounded-full`}>
              {tag}
              </div>
              <h4 className="m-0 text-xl font-bold text-gray-700">{value.title}</h4>
              <p className="text-base text-left text-gray-500 ">{value.description}</p>
              <Date className="text-sm md:text-base font-normal text-gray-600" dateString={value.datePublished} />
            </a>
            </Link>
          ))}
           </div> */}
           <h2 className="mt-10 text-4xl font-bold text-gray-600 text-left">Other popular articles and sources : -</h2>
           <hr className="my-4" />
           <div className="flex flex-wrap flex-row gap-2">
            <div className="article-section1"> 
          <h1 className="mb-0 mt-10 text-6xl font-bold tracking-tight text-left"><span className="bg-clip-text text-transparent bg-gradient-to-r from-green-400 to-green-600 "> Tech </span><span className="bg-clip-text text-transparent bg-gradient-to-r from-red-400 to-pink-700 
          "> Crunch.</span></h1>
        {articles.map((article, index) => (
            <div key={index} class="max-w-sm mt-4 bg-white rounded-xl shadow-md overflow-hidden md:max-w-3xl">
            <div class="md:flex">
          ...
          {/* <div className="article-section1">
        <h1 className="mb-5 mt-10 text-6xl font-bold tracking-tight text-left"><span className="bg-clip-text text-transparent bg-gradient-to-r from-green-400 to-green-600 "> Tech </span><span className="bg-clip-text text-transparent bg-gradient-to-r from-red-400 to-pink-700 
        "> Crunch.</span></h1>
        {articles.map((article, index) => (
            <div key={index} class="max-w-sm mt-4 bg-white rounded-xl shadow-md overflow-hidden md:max-w-3xl">
            <div class="md:flex">
...
export async function getStaticProps() {
  ...
  const TechCrunchapiResponse = await fetch(`https://newsapi.org/v2/top-headlines?sources=techcrunch&pageSize=70`,
    {
      headers: {
